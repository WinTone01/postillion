...
use postillion_doc::{MessagePart, SessionCommandPayload};
use postillion_proto::{ChatConfig, EngineInfo, HarnessId, ToolCall, WorkspaceScope};
use postillion_rpc::{LinkCache, RpcError, RpcReply, RpcService, methods, parse_params};
...
struct RelayCommandParams {
    ...
    entry: postillion_doc::SessionCommandEntry,
}
...
impl EngineRpc {
    ...
    pub fn with_updater(mut self, updater: postillion_update::Updater) -> Self {
    ...
    fn updater(&self) -> Result<&postillion_update::Updater, RpcError> {
        self.updater
            .as_ref()
...
/// The transcript watch as delta frames (`postillion_doc::transcript_delta`): a
...
fn doc_messages_stream(
    rx: watch::Receiver<Vec<postillion_doc::SessionMessageEntry>>,
) -> BoxStream<'static, serde_json::Value> {
    use postillion_doc::transcript_delta::{TranscriptFrame, diff_transcript};
    futures::stream::unfold(
        (rx, None::<Vec<postillion_doc::SessionMessageEntry>>),
...
impl RpcService for EngineRpc {
    async fn handle(&self, method: &str, params: serde_json::Value) -> Result<RpcReply, RpcError> {
        ...
        match method {
            ...
            methods::SYNC_STATUS => {
                fn room_json(s: &postillion_sync::RoomStatsSnapshot) -> serde_json::Value {
                    serde_json::json!({
                ...
                fn chat2_json(s: &postillion_sync::ChatStatsSnapshot) -> serde_json::Value {
                    serde_json::json!({
                        "connected": s.connected,
                        "cursor": s.cursor,
                        "headSeq": s.head_seq,
            ...
            methods::GET_CHECKOUT_DIFF => {
                ...
                Box::pin(async move {
                    ...
                    .map_err(|e| RpcError::Failed(e.to_string()))?;
                    RpcReply::value(&postillion_proto::CheckoutDiff {
            ...
            methods::GET_CHECKOUT_FILE_DIFF_TEXT => {
                ...
                Box::pin(async move {
                    let p: postillion_proto::GetCheckoutFileDiffTextRequest = parse_params(params)?;
                    let identity =
                    ...
                    let stale = || postillion_proto::CheckoutFileDiffText {
                    ...
                    RpcReply::value(&postillion_proto::CheckoutFileDiffText {
            ...
            methods::LIST_MCP_SERVERS => {
                ...
                let names: Vec<String> =
                    postillion_harness::claude::mcp::default_config_path()
                        .map(|path| postillion_harness::claude::mcp::definitions(&path))
                        .unwrap_or_default()
                        .into_keys()
                        .collect();
            ...
            methods::LIST_DRIVES => {
                let drives = self
                    ...
                    .await
                    .map_err(|e| RpcError::Failed(e.to_string()))?;
                RpcReply::value(&postillion_proto::DriveListing { drives })
            }
            methods::SEARCH_FILES => {
