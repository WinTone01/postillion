...
use postillion_proto::{HarnessId, UserInputAnswer, UserInputQuestion};
...
/// beyond this (and beyond [`postillion_doc::DOC_LRU_BYTE_BUDGET`]) is evicted
...
/// (which never expire) ride the same seam as a [`postillion_rpc::StaticToken`].
#[derive(Clone)]
pub struct EdgeConfig {
    ...
    pub token: Arc<dyn postillion_rpc::TokenSource>,
...
impl EdgeConfig {
    pub fn new(url: impl Into<String>, token: Arc<dyn postillion_rpc::TokenSource>) -> Self {
        Self {
            url: url.into(),
            token,
    ...
    pub fn with_static_token(url: impl Into<String>, token: impl Into<String>) -> Self {
        Self::new(url, Arc::new(postillion_rpc::StaticToken(token.into())))
    }
    ...
    pub fn room_url(&self, path: impl Into<String>) -> Arc<dyn postillion_sync::UrlProvider> {
...
struct EdgeRoomUrl {
    base: String,
    token: Arc<dyn postillion_rpc::TokenSource>,
    device_id: String,
}

impl postillion_sync::UrlProvider for EdgeRoomUrl {
    fn url(&self) -> futures::future::BoxFuture<'static, Result<String, postillion_sync::SyncError>> {
        let token = self.token.clone();
        let base = self.base.clone();
        let device = self.device_id.clone();
        Box::pin(async move {
            let token = token.token().await.ok_or_else(|| {
                postillion_sync::SyncError::Auth("no access token (signed out)".into())
            })?;
...
struct DocHostInner {
    ...
    connectivity: OnceLock<watch::Sender<postillion_proto::Connectivity>>,
    ...
    links: OnceLock<Arc<postillion_rpc::LinkCache>>,
...
pub struct ChatDocHandle {
    ...
    chat2: Mutex<Option<postillion_sync::ChatClient>>,
...
impl DocHost {
    ...
    /// Run commands carrying a [`postillion_proto::WorktreeSpec`].
    ...
    pub fn set_links(&self, links: Arc<postillion_rpc::LinkCache>) {
    ...
    fn spawn_migration_sweep(&self) {
        ...
        self.spawn_worker(async move {
            ...
            loop {
                ...
                let chats: Vec<postillion_proto::Chat> = ws.watch_chats().borrow().clone();
    ...
    fn spawn_cutover_watcher(&self, mut chats: watch::Receiver<Vec<postillion_proto::Chat>>) {
        let host = self.clone();
        self.spawn_worker(async move {
            loop {
        ...
        self.spawn_worker(async move {
            ...
            loop {
                ...
                let dial = tokio::time::timeout(
                    ...
                    postillion_sync::ChatClient::connect_via_transport(
                        url.clone(),
                        sink.clone(),
                        fetcher.clone(),
                        &device,
                ...
                match dial {
                    Ok(Ok(client)) => {
                        ...
                        if host.is_host(&chat) {
                            ...
                            let chat = chat.clone();
                            host.clone().spawn_worker(async move {
                                use postillion_sync::chat_client::ChatEvent;
                                loop {
    ...
    async fn seed_chat2(
        ...
        let rebuilt = postillion_doc::rebuild::rebuild_thin_doc(&doc).map_err(|e| e.to_string())?;
    ...
    async fn salvage_chat_transcript(&self, chat_id: &str) -> Result<(), String> {
        ...
        let rebuilt = postillion_doc::rebuild::rebuild_thin_doc(&fat).map_err(|e| e.to_string())?;
    ...
    async fn chat2_maintenance(&self, handle: &Arc<ChatDocHandle>) {
        ...
        if let Ok(tail) =
            postillion_doc::materialize_tail(&handle.doc, now_ms(), postillion_doc::TAIL_MESSAGE_COUNT)
    ...
    fn evict_over_budget(&self) {
        ...
        for (last_access, chat_id) in by_age {
            ...
            if count <= WARM_DOC_CAP && estimate <= postillion_doc::DOC_LRU_BYTE_BUDGET {
                return;
            }
            let evicted = {
                let mut handles = lock(&self.inner.handles);
    ...
    pub fn probe_edge_reachability(&self) {
        ...
        self.spawn_worker_on(&runtime, async move {
            ...
            {
                postillion_sync::wake::notify_online();
            }
        });
    }
    ...
    pub fn watch_connectivity(&self) -> watch::Receiver<postillion_proto::Connectivity> {
        let tx = self
            .inner
            .connectivity
    ...
    fn compute_connectivity(&self) -> postillion_proto::Connectivity {
        use postillion_proto::{ChatConnectivity, Connectivity, ConnectivityState};
        ...
        let path_offline = grace.degraded(
            GraceKey::OsPath,
            postillion_sync::wake::path_is_offline(),
            now,
        );
    ...
    /// Per-open-chat room introspection for SyncStatus / `postillion sync`.
    /// `None` room = still dialing (join retry loop) or edge-less.
    pub fn sync_statuses(&self) -> Vec<(String, Option<postillion_sync::ChatStatsSnapshot>)> {
        let handles: Vec<Arc<ChatDocHandle>> =
            lock(&self.inner.handles).values().cloned().collect();
        let mut rows: Vec<(String, Option<postillion_sync::ChatStatsSnapshot>)> = handles
            .iter()
            .map(|h| {
                (
                    h.chat_id.clone(),
    ...
    ///    the entry itself ([`postillion_rpc::methods::RELAY_COMMAND`]). The
    ...
    async fn relay_command(
        ...
        let supported = self
            ...
            .flatten()
            .find(|d| d.id == target)
            .and_then(|d| d.version.as_deref().and_then(postillion_proto::version_triple))
        ...
        let call = client.call(postillion_rpc::methods::RELAY_COMMAND, params);
        match tokio::time::timeout(RELAY_CALL_TIMEOUT, call).await {
            ...
            Ok(Err(postillion_rpc::RpcError::Failed(err))) => Err(format!("host refused: {err}")),
            Ok(Err(err)) => {
                links.invalidate(target);
    ...
    /// Host side of [`postillion_rpc::methods::RELAY_COMMAND`]: evaluate the
    ...
    async fn push_attachments(
        ...
        for transfer in transfers {
            ...
            loop {
                ...
                let call = client.call(postillion_rpc::methods::UPLOAD_CHUNK, params);
                match tokio::time::timeout(TRANSFER_CHUNK_TIMEOUT, call).await {
                    ...
                    Ok(Err(postillion_rpc::RpcError::Failed(err))) => {
            ...
            let call = client.call(postillion_rpc::methods::UPLOAD_COMMIT, params);
    ...
    pub fn upload_tool_sidecar(&self, chat_id: &str, payload: postillion_doc::SidecarPayload) {
        let Some(edge) = self.inner.config.edge.clone() else {
            return;
        };
        let Ok(runtime) = tokio::runtime::Handle::try_current() else {
    ...
    pub(crate) fn harness_for_request(
        &self,
        chat_id: &str,
        request: &postillion_proto::RunRequest,
    ) -> HarnessId {
    ...
    fn resolve_request_attachments(&self, request: &mut postillion_proto::RunRequest) {
        let Some(uploads) = self.inner.uploads.get() else {
            return;
        };
    ...
    /// Create (or reuse) the isolated worktree a Run's [`postillion_proto::WorktreeSpec`]
    ...
    async fn materialize_worktree(
        &self,
        chat_id: &str,
        spec: &postillion_proto::WorktreeSpec,
    ) -> Result<(String, Option<postillion_proto::Worktree>), EngineError> {
        if let Some(ws) = self.workspace()
    ...
    pub(crate) fn request_from_chat_row(
        ...
        chat_id: &str,
        prompt: &str,
    ) -> Option<postillion_proto::RunRequest> {
        let workspace = self.workspace()?;
        ...
        Some(postillion_proto::RunRequest {
            ...
            sandbox: config
                .as_ref()
                .map(|c| c.sandbox)
                .unwrap_or(postillion_proto::SandboxLevel::WorkspaceWrite),
            auto_approve: false,
            attachments: Vec::new(),
            resume: None,
            worktree: None,
