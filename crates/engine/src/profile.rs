...
mod tests {
    ...
    async fn profile_assembler_uses_the_resolved_local_roots() {
        ...
        let core = crate::EngineCore::assemble_with_profile(
            ...
            postillion_proto::HarnessId::Mock,
            None,
        )
        .unwrap();
    ...
    async fn development_constructor_uses_account_scoped_uploads() {
        let dir = tempfile::tempdir().unwrap();
        let org_id = crate::env_or("POSTILLION_ORG_ID", crate::DEFAULT_ORG_ID);
        let user_id = crate::env_or("POSTILLION_USER_ID", crate::DEFAULT_USER_ID);
        ...
        if std::env::var("POSTILLION_ORG_ID").is_err() && std::env::var("POSTILLION_USER_ID").is_err() {
            assert_eq!(
                expected.store_root(),
