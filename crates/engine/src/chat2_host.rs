...
//! of [`postillion_sync::chat_client::ChatDocSink`] and
//! [`postillion_sync::chat_client::CheckpointFetcher`], binding a
...
use postillion_doc::SessionDoc;
use postillion_sync::chat_client::{ChatDocSink, CheckpointFetcher};
use postillion_sync::{DocsStore, SyncError};
...
impl postillion_sync::chat_client::ChatTransport for EdgeChatTransport {
...
mod frontier_tests {
    ...
    fn encoded_empty_frontier_is_not_contained() {
        let dir = std::env::temp_dir().join(format!("postillion-frontier2-{}", std::process::id()));
    ...
    fn empty_frontier_is_not_contained() {
        let dir = std::env::temp_dir().join(format!("postillion-frontier-test-{}", std::process::id()));
        let store = Arc::new(DocsStore::open(&dir).expect("store opens"));
        let doc = Arc::new(SessionDoc::from_doc(loro::LoroDoc::new()));
