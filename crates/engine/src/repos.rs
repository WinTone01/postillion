...
//! matching `postillion/<name>` branch. `POSTILLION_WORKTREES_DIR` overrides the root.
//!
...
const NOUNS: &[&str] = &[
    "otter", "harbor", "falcon", "cedar", "meadow", "postillion", "delta", "ember", "lynx", "maple",
...
impl Repos {
    ...
    /// remotes, and tags seed the walk, so Postillion's internal refs never leak
    ...
    /// `{worktrees_root}/<repoName>/<generatedName>`, on a fresh `postillion/<name>`
    /// branch off `branch`.
    pub async fn create_worktree(
        ...
        for attempt in 0..50u64 {
            ...
            if !base.join(&candidate).exists() && !existing.contains(&format!("postillion/{candidate}"))
            {
                name = Some(candidate);
                break;
            }
        ...
        let branch_name = format!("postillion/{name}");
        self.git(
            &[
                "worktree",
                "add",
    ...
    /// Rename a postillion-created worktree branch after its chat's generated title
    /// (port of Comet's `renameWorktreeBranch`). Guards:
    /// - respect an external checkout/rename: only act while the worktree is still
    ///   on `expected_branch` AND that branch is the original `postillion/<folderName>`;
    ...
    pub async fn rename_worktree_branch(
        ...
        let folder = worktree_path
            ...
            .map(|n| n.to_string_lossy().to_string())
            .unwrap_or_default();
        if current != expected_branch || expected_branch != format!("postillion/{folder}") {
            return Ok(current);
        }
    ...
    /// Deletes the worktree's branch ONLY when postillion created it (`postillion/…`) — the
    ...
    pub async fn delete_worktree(
        ...
        if branch.starts_with("postillion/") {
    ...
    /// timeout the thread is simply abandoned (the postillion backend's disposable
...
fn list_folders_blocking(target: &Path) -> Result<FolderListing, EngineError> {
    let read = std::fs::read_dir(target).map_err(|e| match e.kind() {
        std::io::ErrorKind::PermissionDenied => {
            EngineError::Other("Postillion doesn't have access to this folder on the device.".into())
        }
...
/// Turn a generated chat title into the semantic portion of a Postillion branch
/// (port of Comet's `worktreeBranchFromTitle`). Postillion NFKD-normalizes accented
...
pub fn worktree_branch_from_title(title: &str) -> String {
    ...
    format!("postillion/{}", if slug.is_empty() { "update" } else { slug })
}
