//! postillion-engine — the headless backend: sessions engine, doc host + command executor,
...
pub use postillion_proto::{EngineInfo, HarnessId, WorkspaceScope};
use postillion_rpc::{RpcError, RpcReply, RpcService, methods};
...
pub enum EngineError {
    #[error("doc: {0}")]
    Doc(#[from] postillion_doc::DocError),
    #[error("journal: {0}")]
    Journal(#[from] run_journal::JournalError),
    #[error("store: {0}")]
    Store(#[from] postillion_sync::StoreError),
    #[error("harness: {0}")]
    Harness(#[from] postillion_harness::HarnessError),
    #[error("io: {0}")]
...
pub struct EngineConfig {
    /// Data directory (default `~/.postillion`, dev `~/.postillion-dev`).
    ...
    /// Workspace-doc org (`ws/{orgId}` room). `None` = `$POSTILLION_ORG_ID` or the dev default.
...
pub struct EngineCore {
    ...
    links: std::sync::Mutex<Option<Arc<postillion_rpc::LinkCache>>>,
    /// Release checker (attached by [`Engine::assemble_runtime`]) — the
    /// UpdateStatus stream + ApplyUpdate.
    updater: std::sync::Mutex<Option<postillion_update::Updater>>,
...
impl EngineCore {
    ...
    /// `$POSTILLION_ORG_ID` / `$POSTILLION_USER_ID` (dev defaults `dev-org` / `dev-user`);
    ...
    pub fn assemble(
        ...
        let org_id = env_or("POSTILLION_ORG_ID", DEFAULT_ORG_ID);
        let user_id = env_or("POSTILLION_USER_ID", DEFAULT_USER_ID);
    ...
    pub fn auth(&self) -> Auth {
        ...
        slot.get_or_insert_with(|| {
            let dev_user = std::env::var("POSTILLION_EDGE_TOKEN")
                .ok()
    ...
    pub fn links(&self) -> Option<Arc<postillion_rpc::LinkCache>> {
        self.links
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .clone()
    ...
    pub fn set_updater(&self, updater: postillion_update::Updater) {
        *self
            .updater
            .lock()
    ...
    pub fn updater(&self) -> Option<postillion_update::Updater> {
        self.updater
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .clone()
    ...
    pub async fn dial_device(
        &self,
        device_id: &str,
    ) -> Result<Arc<postillion_rpc::RpcClient>, EngineError> {
        let links = self
            .links()
    ...
    pub fn start_host_relay(&self, edge_url: &str) -> postillion_rpc::HostRelay {
        let auth = self.auth();
        let config =
            postillion_rpc::HostRelayConfig::new(edge_url, self.device_id.clone(), Arc::new(auth));
        let doc_host = self.doc_host.clone();
        let on_nudge: postillion_rpc::NudgeHandler = Arc::new(move |chat_id: String| {
        ...
        postillion_rpc::HostRelay::spawn(config, self.rpc_service(), on_nudge)
    }
...
pub struct EngineRuntime {
    core: EngineCore,
    host_relay: std::sync::Mutex<Option<postillion_rpc::HostRelay>>,
}

/// IPC-only lifecycle control owned by `postillion headless`. The regular
...
impl Engine {
    ...
    pub async fn build_auth(config: &EngineConfig) -> Auth {
        ...
        if let Ok(base) = std::env::var("POSTILLION_WORKOS_API_BASE")
            && !base.trim().is_empty()
        {
            auth_config.workos_api_base = base;
        }
        auth_config.callback_port = Some(
            std::env::var("POSTILLION_CALLBACK_PORT")
                .ok()
        ...
        match scope {
            ...
            WorkspaceScope::Development => {
                ...
                let org_id = dev_token_org
                    ...
                    .unwrap_or_else(|| env_or("POSTILLION_ORG_ID", DEFAULT_ORG_ID));
                let user_id = auth
                    .user_id()
                    .unwrap_or_else(|| env_or("POSTILLION_USER_ID", DEFAULT_USER_ID));
    ...
    async fn assemble_runtime_inner(
        ...
        let edge_enabled = match profile.scope() {
            ...
            // POSTILLION_WORKOS_CLIENT_ID="". Only an explicitly configured,
        ...
        if edge_enabled {
            ...
            postillion_sync::net_path::spawn_path_monitor();
        }
        ...
        if edge_enabled {
            ...
            // installs with POSTILLION_AUTO_UPDATE=1 apply + restart themselves — gated
            // on quiescence so a restart never lands under a live run or open PTY.
            let quiescent: postillion_update::QuiescentCheck = {
                let sessions = core.sessions.clone();
                let terminals = core.terminals.clone();
                Arc::new(move || !sessions.any_active() && !terminals.any_open())
            };
            let updater = postillion_update::Updater::spawn(config.edge_url.clone(), Some(quiescent));
        ...
        postillion_harness::acp::prewarm_managed_adapters();

        let host_relay = edge.as_ref().map(|edge| {
            let mut link_config =
                postillion_rpc::LinkCacheConfig::new(edge.url.clone(), Arc::new(auth.clone()));
            ...
            let links = postillion_rpc::LinkCache::new(link_config);
...
pub async fn serve_ipc(
    port: u16,
    service: std::sync::Arc<dyn postillion_rpc::RpcService>,
) -> std::io::Result<tokio::task::JoinHandle<()>> {
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", port)).await?;
    tracing::info!(port, "IPC server listening");
    Ok(tokio::spawn(postillion_rpc::serve_ws_listener(
        listener, service,
    )))
}
...
/// `postillion login` persisted, never wait on a prompt nobody can see.
    ...
    loop {
        ...
        match state {
            ...
            AuthState::NeedsOrganization { user } => {
                if !interactive {
                    ...
                    return Err(EngineError::Other(format!(
                        "signed in as {} but no workspace is selected — run `postillion login` on this machine to pick one",
                        user.email
                    )));
                }
            ...
            AuthState::SignedOut => {
                if !interactive {
                    return Err(EngineError::Other(
                        "not signed in — run `postillion login` on this machine first".into(),
                    ));
                }
...
fn local_device_name(device_id: &str) -> String {
    select_local_device_name(
        [
            std::env::var("POSTILLION_DEVICE_NAME").ok(),
