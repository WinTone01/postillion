...
use postillion_engine::{EngineCore, HarnessRegistry, RunJournal};
use postillion_harness::mock::MockHarness;
...
fn queue_as_viewer(doc: &SessionDoc, id: &str, payload: SessionCommandPayload) {
    ...
    let based_on =
        doc.read_entries()
            .expect("read entries")
            .last()
            .map(|m| postillion_doc::CommandBasedOn {
...
async fn processed_commands_are_skipped_on_redelivery() {
    ...
    let verdict = postillion_doc::evaluate_command(
        entry,
        &postillion_doc::EvaluationContext {
    ...
    assert_eq!(verdict, postillion_doc::CommandDisposition::Skip);
}
...
async fn rpc_surface_over_in_memory_transport() {
    ...
    let harnesses = client
        .call(postillion_rpc::methods::LIST_HARNESSES, serde_json::Value::Null)
        .await
        .unwrap();
    assert_eq!(harnesses[0]["id"], "mock");
    let models = client
        .call(
            postillion_rpc::methods::LIST_MODELS,
    ...
    let mut sessions_stream = client
        .subscribe(postillion_rpc::methods::WATCH_SESSIONS, serde_json::Value::Null)
        .await
        .unwrap();
    ...
    let mut messages_stream = client
        .subscribe(
            postillion_rpc::methods::WATCH_DOC_MESSAGES,
    ...
    let queued = client
        .call(
            postillion_rpc::methods::QUEUE_COMMAND,
    ...
    let settled = loop {
        let item = tokio::time::timeout_at(deadline, messages_stream.recv())
            ...
            .expect("stream alive");
        let frame: postillion_doc::TranscriptFrame = serde_json::from_value(item).unwrap();
        postillion_doc::apply_transcript_frame(&mut materialized, frame).unwrap();
...
async fn respond_input_resolves_pending_question() {
    ...
    impl Harness for AskingHarness {
        ...
        async fn run(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<Result<AgentEvent, HarnessError>>(16);
            tokio::spawn(async move {
                let answers = (controls.request_input)(vec![postillion_proto::UserInputQuestion {
                    id: "q1".into(),
                    header: "Pick".into(),
    ...
    queue_as_viewer(
        ...
        SessionCommandPayload::RespondInput {
            request_id,
            answers: vec![postillion_proto::UserInputAnswer {
                question_id: "q1".into(),
                labels: vec!["b".into()],
            }],
        },
        ...
        async fn run(
            ...
            if request.prompt == "second run" {
                ...
                tokio::spawn(async move {
                    ...
                    let _ = (controls.request_input)(vec![postillion_proto::UserInputQuestion {
                        id: "q1".into(),
                        header: "Pick".into(),
                        question: "Which one?".into(),
                        options: vec!["a".into(), "b".into()],
...
async fn harness_emitted_input_twin_is_dropped_and_answer_resumes() {
    ...
    impl Harness for DoubleEmitHarness {
        ...
        async fn run(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<Result<AgentEvent, HarnessError>>(16);
            tokio::spawn(async move {
                let question = postillion_proto::UserInputQuestion {
                    id: "q1".into(),
                    header: "Pick".into(),
                    question: "Which one?".into(),
                    options: vec!["a".into(), "b".into()],
...
async fn attachment_upload_then_run_threads_refs_and_paths() {
    ...
    for (seq, data) in [(0, first), (1, second)] {
        client
            .call(
                postillion_rpc::methods::UPLOAD_CHUNK,
    ...
    let committed = client
        .call(
            postillion_rpc::methods::UPLOAD_COMMIT,
    ...
    let chunk = client
        .call(
            postillion_rpc::methods::READ_ATTACHMENT_CHUNK,
...
/// Run with: `cargo test -p postillion-engine --test e2e -- --ignored`
#[tokio::test(flavor = "multi_thread")]
...
async fn real_claude_sees_uploaded_image_inline() {
    ...
    let core = EngineCore::assemble(
        &dir,
        Arc::new(postillion_engine::default_registry()),
