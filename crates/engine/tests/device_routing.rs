...
use postillion_doc::SessionCommandPayload;
use postillion_engine::{
...
async fn checkout_change_request_stream_matches_locally_and_through_device_routing() {
    ...
    let local_client = postillion_rpc::memory_client(core_b.rpc_service());
    ...
    let client = postillion_rpc::memory_client(core_a.rpc_service());
