...
//! variant against a real edge runs behind `#[ignore]` (POSTILLION_EDGE_WS, like
//! postillion-sync's edge_convergence test).
...
use postillion_doc::{CommandBasedOn, SessionCommandEntry, SessionCommandPayload, SessionCommandStatus};
...
async fn bridge(
    a: &EngineCore,
    b: &EngineCore,
) -> postillion_sync::registry::mock_server::MockRegistryServer {
    let server = postillion_sync::registry::mock_server::MockRegistryServer::start().await;
...
async fn two_engines_share_a_workspace() {
    ...
    let client_a = postillion_rpc::memory_client(a.rpc_service());
    let client_b = postillion_rpc::memory_client(b.rpc_service());
    client_a
        .call(
            methods::MUTATE,
            serde_json::json!({
    ...
    wait_for(
        || {
            handle.doc().read_entries().unwrap_or_default().iter().any(|e| {
                e.parts.iter().any(
                    |p| matches!(p, postillion_doc::MessagePart::Text { text, .. } if text == "From cursor"),
                )
            })
        },
...
///
/// ```sh
/// POSTILLION_EDGE_WS=ws://127.0.0.1:8787 cargo test -p postillion-engine -- --ignored
/// ```
#[tokio::test]
#[ignore = "requires a live edge: set POSTILLION_EDGE_WS (e.g. ws://127.0.0.1:8787)"]
async fn two_engines_converge_through_a_real_workspace_room() {
    use postillion_engine::doc_host::EdgeConfig;

    let base = std::env::var("POSTILLION_EDGE_WS")
        .expect("set POSTILLION_EDGE_WS to the edge origin, e.g. ws://127.0.0.1:8787");
...
async fn legacy_workspace_doc_migrates_instantly_on_first_boot() {
    use postillion_proto::{Chat, Device, Session, Space};
    ...
    {
        let store = postillion_sync::DocsStore::open(&org_dir).expect("open store");
        let legacy = postillion_doc::WorkspaceDoc::new();
    ...
    let store = postillion_sync::DocsStore::open(&org_dir).expect("reopen store");
    assert!(
        store
            .load_snapshot(postillion_doc::REGISTRY_DOC_ID)
