// postillion cursor shim — a thin, postillion-owned wrapper around the PINNED
...
// every bump). Materialized by postillion into the SDK's managed install dir so
...
// Postillion runs concurrent shims on the same cwd as a matter of course — the
...
const STATE_BASE =
  process.env.POSTILLION_CURSOR_STATE_DIR || path.join(os.homedir(), ".postillion", "cursor-state");
...
if (process.argv[2] === "login") {
  ...
  try {
    const result = await Cursor.auth.login({
      ...
      apiKeyName: `postillion — ${os.hostname()}`,
    });
    out({
      ev: "logged-in",
...
async function start(msg) {
  ...
  const options = {
    ...
    // generateImage has nowhere to land in a postillion session (ACP parity).
