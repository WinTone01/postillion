...
//!
//! Set `POSTILLION_NO_LOGIN_SHELL=1` to disable the snapshot entirely.
...
mod unix {
    ...
    const BEGIN_MARKER: &str = "__POSTILLION_SHELL_ENV_BEGIN__";
    const END_MARKER: &str = "__POSTILLION_SHELL_ENV_END__";
    ...
    pub(super) fn capture() -> Option<OsString> {
        if std::env::var_os("POSTILLION_NO_LOGIN_SHELL").is_some_and(|v| !v.is_empty()) {
            return None;
        }
    ...
    fn run_and_capture(shell: &Path, flags: &[&str], script: &str, timeout: Duration) -> Vec<u8> {
        ...
        cmd.args(flags)
            ...
            .env("POSTILLION_RESOLVING_ENVIRONMENT", "1")
            .env("TERM", "dumb");
