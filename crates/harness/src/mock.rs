...
/// The scripted question set for the `POSTILLION_MOCK_QUESTION` variant (exercises
...
impl Harness for MockHarness {
    ...
    async fn run(
        ...
        // Optional pacing knob for demos/manual testing: `POSTILLION_MOCK_DELAY_MS`
        ...
        let delay_ms = std::env::var("POSTILLION_MOCK_DELAY_MS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(0);
        let delay = std::time::Duration::from_millis(delay_ms);

        // Dev/testing knob: `POSTILLION_MOCK_QUESTION=1` swaps in a run that asks
        ...
        let question_mode = std::env::var("POSTILLION_MOCK_QUESTION")
            .ok()
        ...
        // Dev/testing knob: `POSTILLION_MOCK_REPEAT=N` loops the script body N times
        ...
        // at the very end).
        let repeat = std::env::var("POSTILLION_MOCK_REPEAT")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .unwrap_or(1)
            .max(1);
        // Dev/testing knob: `POSTILLION_MOCK_ERROR=1` appends a scripted error
        // before the terminal Done — the only data-side way to put the
        // transcript ErrorChip on screen with the mock harness.
        let mock_error = std::env::var("POSTILLION_MOCK_ERROR")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        // Dev/testing knob: `POSTILLION_MOCK_TABLE=1` appends scripted GFM tables
        ...
        let mock_table = std::env::var("POSTILLION_MOCK_TABLE")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        let done_ix = self
            .script
        ...
        // Dev/testing knob: `POSTILLION_MOCK_CODE=1` appends rust + ts code blocks
        ...
        let mock_code = std::env::var("POSTILLION_MOCK_CODE")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        ...
        // Dev/testing knob: `POSTILLION_MOCK_MEND=1` appends a link/list-heavy
        ...
        let mock_mend = std::env::var("POSTILLION_MOCK_MEND")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
        ...
        // Dev/testing knob: `POSTILLION_MOCK_SUBAGENT=1` appends two spawn chips
        ...
        // paced run (`POSTILLION_MOCK_DELAY_MS`) holds a Running chip long enough
        // to observe.
        let mock_subagent = std::env::var("POSTILLION_MOCK_SUBAGENT")
            .ok()
            .is_some_and(|v| !v.is_empty() && v != "0");
            ...
            .then(|| {
                ...
                vec![
                    ...
                    tag(
                        ...
                        AgentEvent::ToolCall {
                            id: "sub1-grep".into(),
                            call: postillion_proto::ToolCall::Exec {
                    ...
                    tag(
                        ...
                        AgentEvent::ToolCall {
                            id: "sub2-burst".into(),
                            call: postillion_proto::ToolCall::Exec {
                                command: "cargo test -p postillion-doc cadence_burst -- --nocapture".into(),
                            },
                        },
                    ),
                    resolve("mock-sub-2"),
                    ...
                    tag(
                        ...
                        AgentEvent::ToolCall {
                            ...
                            call: postillion_proto::ToolCall::Exec {
                                command: "cargo test -p postillion-doc cadence_steer -- --nocapture"
                                    .into(),
                            },
                        },
                    ),
        ...
        let events: Vec<Result<AgentEvent, HarnessError>> = body
            ...
            .map(Ok)
            .collect();
        // Dev/testing knob: `POSTILLION_MOCK_CHARS=N` re-chunks every TextDelta
        // into N-char deltas, so `POSTILLION_MOCK_DELAY_MS` paces *characters*
        ...
        let chunk_chars = std::env::var("POSTILLION_MOCK_CHARS")
            .ok()
            .and_then(|v| v.parse::<usize>().ok())
            .filter(|&n| n > 0);
        ...
        // Dev/testing knob: `POSTILLION_MOCK_SUBAGENT_DELAY_MS` paces TAGGED
        // (subagent) events on their own clock — the parent turn settles at
        // `POSTILLION_MOCK_DELAY_MS` speed while the background subagents stream
        ...
        let sub_delay_ms = std::env::var("POSTILLION_MOCK_SUBAGENT_DELAY_MS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok());
