...
use postillion_harness::{AcpHarness, Harness as _};
...
async fn cli_on_login_shell_path_only_is_resolved() {
    ...
    let snapshot = postillion_harness::shell_env::login_shell_path().expect("snapshot captured");
    ...
    assert!(
        postillion_harness::ClaudeHarness::new().installed(),
