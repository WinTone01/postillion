...
//!
//! Single-test binary: it mutates POSTILLION_ADAPTERS_DIR process-wide.

use futures::StreamExt;
use tokio::sync::mpsc;
use tokio_util::sync::CancellationToken;
use postillion_harness::{AcpHarness, Harness, RunControls};
use postillion_proto::{AgentEvent, RunRequest};
...
async fn managed_install_reaches_session_started() {
    ...
    unsafe {
        std::env::set_var("POSTILLION_ADAPTERS_DIR", adapters.path());
    ...
    let version_dir = std::fs::read_dir(&root)
        ...
        .path();
    assert!(version_dir.join(".postillion-install-ok").exists());
}
