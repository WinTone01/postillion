...
//!
//! Single-test binary: it mutates PATH/SHELL/POSTILLION_* env process-wide.
...
use postillion_harness::{AcpHarness, Harness, HarnessError, RunControls};
use postillion_proto::{RunRequest, SandboxLevel};
...
async fn silent_npm_enoent_death_surfaces_decoded_error() {
    ...
    unsafe {
        std::env::set_var("POSTILLION_ADAPTERS_DIR", dir.path().join("adapters"));
        std::env::set_var("POSTILLION_NO_LOGIN_SHELL", "1");
