...
//! (`postillion-ui/src/motion.rs`, `postillion-ui/src/loaders.rs`), lifted here so any
...
/// Postillion loader cell opacity for a phase: 0.08 → 1 → 0.08.
pub fn pulse_opacity(phase: f32) -> f32 {
    PULSE_MIN_OPACITY + (1.0 - PULSE_MIN_OPACITY) * pulse_wave(phase)
}

/// Postillion loader cell scale for a phase: 0.9 → 1 → 0.9.
...
/// The postillion mark's pixels — `[x, y]` of each 100×100 cell on the 820×940
