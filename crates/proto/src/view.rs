...
/// Session-row sub-line, "project · branch" (postillion `chatLocation`): the repo
...
/// read the same on every surface. `postillion-ui` has the oklch→sRGB math.
pub mod dot {
...
pub enum CheckoutPlan {
    ...
    /// `CreateWorktree` off `base` on send (the engine mints a `postillion/<name>`
