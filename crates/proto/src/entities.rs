...
//!
//! In postillion these were synced Postgres rows; in postillion they live in the per-org
...
pub struct Device {
    ...
    /// First registration time (postillion devices.created_at — the Devices page
...
pub struct Worktree {
    ...
    pub path: String,
    pub branch: String,
    /// Generated worktree folder name (`postillion/<name>` is its branch).
    #[serde(default)]
    pub name: String,
        ...
        for (state, encoded_state) in [
            ...
            let status = CheckoutChangeRequestStatus {
                ...
                change_request: Some(ChangeRequestSummary {
                    ...
                    url: "https://github.com/acme/postillion/pull/90".into(),
                    state,
                    base_ref: "main".into(),
