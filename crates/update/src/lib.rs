//! postillion-update — release checking and self-update, shared by the engine (the
//! background checker + `ApplyUpdate`), the CLI (`postillion update`), and the UI
//! (the sidebar update strip + macOS bundle swap).
//!
//! Release layout (see `.github/workflows/release.yml` and `edge/src/install.sh`):
//! artifacts live in the `postillion-releases` R2 bucket, served pre-auth at
...
//! - **Managed** (`~/.postillion/app/<ver>` + `current` symlink — the curl|sh
...
/// `postillion-<ver>-<os>-<arch>.tar.gz` — the headless/CLI tarball (Linux CI builds).
pub fn headless_artifact(version: &str) -> String {
    let (os, arch) = platform_key();
    format!("postillion-{version}-{os}-{arch}.tar.gz")
}

/// `postillion-<ver>-macos-<arch>-app.tar.gz` — the macOS app update payload.
pub fn mac_app_artifact(version: &str) -> String {
    let (_, arch) = platform_key();
    format!("postillion-{version}-macos-{arch}-app.tar.gz")
}
...
fn http_client() -> anyhow::Result<reqwest::Client> {
    reqwest::Client::builder()
        .user_agent(concat!("postillion/", env!("CARGO_PKG_VERSION")))
        .build()
...
pub enum InstallKind {
    /// `~/.postillion/app/<ver>/postillion` behind the `current` symlink
    /// (curl|sh installer / a previous `postillion update`).
...
fn detect_install_from(exe: &Path, home: Option<&Path>) -> InstallKind {
    if let Some(home) = home {
        ...
        let app_root = home.join(".postillion").join("app");
...
pub async fn stage_headless(
    ...
    if dest.join("postillion").exists() {
        return Ok(dest);
    }
    ...
    let result = async {
        ...
        if !unpacked.join("postillion").is_file() {
            bail!("tarball {file} did not contain a postillion binary");
        }
        match std::fs::rename(&unpacked, &dest) {
            Ok(()) => {}
            // Lost a race with another stager — the staged copy is equivalent.
            Err(_) if dest.join("postillion").exists() => {}
            Err(err) => {
...
pub fn apply_headless(app_root: &Path, version: &str) -> anyhow::Result<()> {
    #[cfg(unix)]
    {
        ...
        if !target.join("postillion").exists() {
...
/// Restart the installed engine service (the same units `postillion daemon` and the
...
pub fn restart_service() -> anyhow::Result<()> {
    if cfg!(target_os = "macos") {
        ...
        run(
            "launchctl",
            &["kickstart", "-k", &format!("gui/{uid}/sh.postillion.app")],
        )
    } else {
        run("systemctl", &["--user", "restart", "postillion.service"])
    }
}
...
/// `POSTILLION_AUTO_UPDATE=1|true|yes` — headless daemons apply updates themselves.
fn auto_update_enabled() -> bool {
    std::env::var("POSTILLION_AUTO_UPDATE")
...
/// stream). Managed installs with `POSTILLION_AUTO_UPDATE` set stage + apply + service
...
mod tests {
    ...
    fn install_kind_detection() {
        assert_eq!(
            detect_install_from(
                Path::new("/home/u/.postillion/app/0.1.1/postillion"),
                Some(Path::new("/home/u")),
            ),
            InstallKind::Managed {
                app_root: PathBuf::from("/home/u/.postillion/app")
            }
        );
        assert_eq!(
            detect_install_from(
        ...
        assert_eq!(
            detect_install_from(Path::new("/tmp/foo.app/postillion"), None),
            InstallKind::Unmanaged
        );
        assert_eq!(
            detect_install_from(
                Path::new("/src/target/release/postillion"),
                Some(Path::new("/home/u"))
            ),
            InstallKind::Unmanaged
        );
    ...
    fn artifact_names_match_packaging() {
        let (os, arch) = platform_key();
        assert!(headless_artifact("0.2.0").starts_with("postillion-0.2.0-"));
        assert_eq!(
            headless_artifact("0.2.0"),
            format!("postillion-0.2.0-{os}-{arch}.tar.gz")
        );
    ...
    fn manifest_parses_with_and_without_files() {
        let full: Manifest = serde_json::from_str(
            r#"{"version":"0.1.1","files":{"postillion-0.1.1-linux-x86_64.tar.gz":{"sha256":"abc"}}}"#,
        )
        .unwrap();
        assert_eq!(full.version, "0.1.1");
        assert_eq!(
            full.files["postillion-0.1.1-linux-x86_64.tar.gz"]
                .sha256
                .as_deref(),
            Some("abc")
        );
    ...
    fn headless_symlink_swap() {
        ...
        for ver in ["0.1.0", "0.1.1"] {
            ...
            std::fs::write(app_root.join(ver).join("postillion"), ver).unwrap();
        }
