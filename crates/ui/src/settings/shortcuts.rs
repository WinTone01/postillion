...
pub struct ShortcutsPage {
    ...
    /// conflicts never persist; they're refused at record time, as in postillion.
    ...
    fn on_key_down(&mut self, event: &KeyDownEvent, cx: &mut Context<Self>) {
        ...
        match record_key(
            ...
            RecordOutcome::Set(combo) => {
                ...
                // (postillion settings.shortcuts.tsx: "… is already assigned to …").
...
/// One-line purpose copy per shortcut (postillion lib/shortcuts.ts
...
impl Render for ShortcutsPage {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let rows = ShortcutId::ALL.into_iter().enumerate().map(|(ix, id)| {
            ...
            // postillion settings.shortcuts.tsx row: min-h-[72px] px-5 gap-5, label
        ...
        // the message names the specific clash (postillion settings.shortcuts.tsx).
...
mod tests {
    ...
    fn conflicting_records_are_refused() {
        // postillion parity: a combo bound elsewhere is refused at record time (the
