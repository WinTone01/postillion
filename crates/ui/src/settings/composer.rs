...
//! (postillion parity: localStorage `postillion.composer.defaults:v1`, defaults.ts).
//!
...
use postillion_proto::{HarnessId, ReasoningLevel};
...
impl ComposerDefaults {
    ...
    /// Remember a pick (postillion `saveDefaults({ harness, modelByHarness })`).
