...
//! account-shaped loading skeletons. Postillion retargets devices from the settings
...
/// Compact absolute reset moment (postillion settings.agents.tsx `formatReset`):
...
/// in the empty-state copy, postillion settings.agents.tsx `PROVIDERS`).
...
impl LoginFlow {
    /// Dialog title (postillion: "Add Claude account" / "Add Codex account").
...
pub struct AccountsPage {
    ...
    /// Retargeted by the page-header device switcher (postillion parity: the
...
impl AccountsPage {
    ...
    /// The page-header device switcher (postillion device-switcher.tsx): a quiet
    ...
    /// One usage window (postillion settings.agents.tsx `UsageMeter`): label ·
                    ...
                    .when(fraction > 0.0, |el| {
                        el.child(
                            div()
                                ...
                                // visible (postillion `max(used, 1.5)%`).
    ...
    /// One account row (postillion settings.agents.tsx `AccountRow`): initial
    ...
    fn render_account_row(
        ...
        // Actions only on INACTIVE accounts (postillion `{!account.active && …}`):
            ...
            .child(
                div()
                    ...
                    .map(|el| {
                        ...
                        // (postillion: `usage ? meters : "Usage unavailable"…`).
    ...
    fn render_login_dialog(
        ...
        let url_link =
            |id: &'static str, label: &'static str, url: &str, cx: &mut Context<Self>| {
                ...
                // "Reopen the …" text link (postillion: `text-[12px]
                ...
                let body = match harness {
                    HarnessId::Cursor => {
                        "Finish signing in to Cursor in your browser. This mints a \
                         postillion-named API key you can revoke any time from Cursor's \
    ...
    /// A ghost account row (postillion settings.agents.tsx `SkeletonRow`): avatar,
...
impl Render for AccountsPage {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Brand mark inside a 24px centered box (postillion: `grid size-6
        ...
        // One section per provider (postillion settings.agents.tsx `ProviderSection`):
            ...
            Loadable::Ready(snapshot) => {
                ...
                PROVIDERS
                    ...
                    .map(|(harness, name, cli)| {
                        ...
                        // EVERY warning renders its own strip (postillion maps them).
            ...
            .child(
                widgets::page_column()
                    .child(
                        div()
                            ...
                            .child(
                                ...
                                // dimmed while a refresh is in flight (postillion
                                // `disabled:opacity-50`).
                                widgets::ghost_action(&theme)
                                    .id("accounts-refresh")
                                    .flex_none()
                    ...
                    .child(widgets::page_subtitle(
                        &theme,
                        "The Claude Code, Codex, and Cursor logins on this device. Postillion \
                    ...
                    .children(sections)
                    // Footer note (postillion: `mt-6 text-[12px] leading-relaxed
...
mod tests {
    ...
    #[test]
    fn usage_thresholds_match_postillion() {
