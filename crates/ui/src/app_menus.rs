...
//!
//! postillion never called `cx.set_menus`, so on macOS `NSApp.mainMenu` stayed nil:
...
pub fn init(cx: &mut App) {
    ...
    // Window verbs route to the active window. postillion is single-window, so a
...
/// The postillion menu bar. macOS renders this natively; mac-only entries are gated
