...
use postillion_syntax::HighlightKind;
...
impl Theme {
    ...
    /// Main-panel header height (postillion `h-11`) — in-card headers (changes pane).
    ...
    /// Reserved status strip under the content outlet (postillion `h-6`) — the
    ...
    /// (see [`crate::frost::frosted`]). Dark: the reference postillion
...
mod tests {
    ...
    #[test]
    fn layout_numbers_match_postillion() {
