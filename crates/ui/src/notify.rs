...
//!   the banner is attributed to Postillion with its icon. The API is deprecated
...
//!   center — those adopt the installed Postillion app's bundle identity (the
//!   terminal-notifier technique: override `-[NSBundle bundleIdentifier]` for
//!   the main bundle), so dev banners still show the Postillion icon; a click may
//!   focus the installed app rather than the dev binary — acceptable for a
//!   dev-only path. Machines without Postillion installed fall back to
...
/// `-[NSBundle bundleIdentifier]` to answer the installed Postillion app's id for
