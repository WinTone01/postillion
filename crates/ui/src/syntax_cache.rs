...
use postillion_syntax::{HighlightedDocument, LanguageId};
    ...
    std::mem::size_of::<HighlightedDocument>()
        .saturating_add(
            document
                .lines
                .capacity()
                .saturating_mul(std::mem::size_of::<Vec<postillion_syntax::HighlightSpan>>()),
        )
        .saturating_add(document.lines.iter().fold(0usize, |total, line| {
            total.saturating_add(
                line.capacity()
                    .saturating_mul(std::mem::size_of::<postillion_syntax::HighlightSpan>()),
            )
        }))
}
