...
//! collapse flags (postillion persisted the same set in localStorage).
//!
...
pub struct UiSettings {
    ...
    /// Suppress the banner while a Postillion window is focused (the chime covers
    /// the foreground case).
    pub notifications_background_only: bool,
    pub right_pane_width: f32,
    /// Legacy: panel *open* flags are session-scoped in-memory state now
    /// (`shell::SessionPanels`, postillion `sessionPanels` parity). Kept for file
...
impl ShortcutId {
    ...
    /// Row label (postillion lib/shortcuts.ts `SHORTCUT_DEFINITIONS`, verbatim).
