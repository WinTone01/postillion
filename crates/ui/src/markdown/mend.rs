...
//! handlers don't apply here — postillion renders raw HTML literally and has no
//! math):
...
pub const PENDING_LINK_URL: &str = "postillion:pending-link";
...
mod tests {
    ...
    fn links_mend_to_pending_sentinel() {
        mends("[docs](https://x.dev/lo", "[docs](postillion:pending-link)");
        mends("[docs](", "[docs](postillion:pending-link)");
        mends("see [do", "see [do](postillion:pending-link)");
        mends("![alt](https://x/i.p", "![alt](postillion:pending-link)");
        stays("see [");
    ...
    fn link_urls_allow_nested_parens() {
        ...
        mends("[a](https://x.dev/(y", "[a](postillion:pending-link)");
    }

    #[test]
    fn emphasis_inside_link_text_closes_inside() {
        mends("[**a", "[**a**](postillion:pending-link)");
        mends("**a [b", "**a [b](postillion:pending-link)**");
    }
