...
use postillion_rpc::{RpcClient, connect_ws, methods};
...
fn apply_transcript_item(
    current: &mut Vec<postillion_doc::SessionMessageEntry>,
    item: &serde_json::Value,
) -> Result<(), String> {
    let frame: postillion_doc::TranscriptFrame =
        serde_json::from_value(item.clone()).map_err(|err| err.to_string())?;
    postillion_doc::apply_transcript_frame(current, frame).map_err(|err| err.to_string())
}
    ...
    let (by_device, text) = wait_stream(
        ...
        |item| {
            ...
            let entry = transcript.iter().find(|entry| {
                entry.role == postillion_doc::MessageRole::Assistant
                    && entry.status == Some(postillion_doc::MessageStatus::Complete)
            })?;
...
mod tests {
    ...
    fn transcript_reset_frame_is_materialized() {
        ...
        assert_eq!(
            transcript[0].status,
            Some(postillion_doc::MessageStatus::Complete)
        );
    }
}
