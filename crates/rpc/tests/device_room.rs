...
fn noop_nudge() -> postillion_rpc::NudgeHandler {
    Arc::new(|_| {})
}
...
async fn nudges_reach_the_host_callback() {
    ...
    let on_nudge: postillion_rpc::NudgeHandler = Arc::new(move |chat_id| {
...
async fn live_edge_relay_round_trip() {
    let Ok(edge_url) = std::env::var("POSTILLION_EDGE_WS") else {
        panic!("set POSTILLION_EDGE_WS to the edge base URL (e.g. http://127.0.0.1:26640)");
    };
    let token = std::env::var("POSTILLION_EDGE_TOKEN").unwrap_or_else(|_| "relay-live-test".into());
...
async fn zombie_relay_path_trips_the_echo_deadline() {
    ...
    // deadline instead.
    postillion_rpc::device_room::set_client_liveness_for_tests(
    ...
    postillion_rpc::device_room::set_client_liveness_for_tests(Duration::ZERO, Duration::ZERO);
}
