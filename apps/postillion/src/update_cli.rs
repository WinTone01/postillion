//! `postillion update` — check for and apply a newer release, natively (the same
...
use anyhow::bail;
use postillion_update::{InstallKind, current_version, version_newer};
...
pub async fn update(edge_url: &str, check_only: bool) -> anyhow::Result<()> {
    ...
    match postillion_update::detect_install() {
        InstallKind::Managed { app_root } => {
            println!(
                "downloading {}…",
                postillion_update::headless_artifact(&manifest.version)
            );
            postillion_update::stage_headless(edge_url, &manifest, &app_root).await?;
            postillion_update::apply_headless(&app_root, &manifest.version)?;
            println!(
                "installed {} (current → {})",
                app_root.join(&manifest.version).display(),
                manifest.version
            );
            match postillion_update::restart_service() {
        ...
        InstallKind::MacApp { bundle } => {
            println!(
                "downloading {}…",
                postillion_update::mac_app_artifact(&manifest.version)
            );
