...
/// postillion-pulse loading row: 5 cells, cosine wave, stagger 0.15/2.4
/// (loaders.rs:91).
struct PostillionPulse: View {
        ...
        TimelineView(.animation(paused: reduceMotion)) { timeline in
            ...
            HStack(spacing: cellSize / 2) {
                ForEach(0..<5, id: \.self) { ix in
                    let phase = (t / Motion.postillionPulsePeriod - Double(ix) * (0.15 / 2.4))
