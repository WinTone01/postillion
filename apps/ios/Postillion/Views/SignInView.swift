...
// The postillion mark on black, one white button — the old mobile app's Gate.
//
...
enum Endpoints {
    // No hosted edge ships with Postillion; point this at your own server.
    static let edgeURL = URL(string: "http://127.0.0.1:8787")!
    // Upstream's AuthKit tenant. Replace with your own before shipping sign-in.
    static let workosClientId = ""
    static let workosAPIBase = "https://api.workos.com"
    static let callbackScheme = "postillion"
...
struct SignInView: View {
    ...
    /// The AuthKit code flow: system browser session → postillion://callback with
...
/// The actual postillion mark — the desktop's 34-cell logo
/// (crates/ui/assets/icons/postillion-logo.svg), cells scaled from its 820×940
/// viewbox and tinted by `color`.
struct PostillionMark: View {
...
struct PostillionMarkShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        for (x, y) in PostillionMark.cells {
