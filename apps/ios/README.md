...
A native SwiftUI viewport onto the postillion mesh. The phone is a **peer
...
```sh
cd apps/ios
xcodebuild -project Postillion.xcodeproj -scheme Postillion \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro' build
```

Or open `Postillion.xcodeproj` in Xcode and run. Dependencies (SPM, resolved
