...
- Add an **ACP harness** (`crates/harness/src/acp/`) speaking Agent Client Protocol
  ...
  NOT the official SDK crates: postillion keeps its own child-lifecycle hardening
...
- First registered agent: **Grok Build** (`grok agent stdio`), xAI's native ACP
  ...
  OAuth or `XAI_API_KEY`; postillion passes env through. `GROK_EXECUTABLE` overrides
...
- **claude/codex converted to ACP too** (2026-08-08, wing's call: "keep things
  ...
  adapter-owned; postillion-specific settings ride config options where advertised
...
- **Hermes + Pi registered** (2026-08-08): `AcpHarness::hermes()` runs Nous
  ...
  generic `thought_level` preference ladder ("off" has no postillion tier).
...
- **Session config options**: ACP has no per-prompt model field; the run's
  ...
  `low`/`medium`/`high`; other postillion levels degrade down a preference ladder
  (`config_option_sets`).
...
- The `npx -y <pinned>` fallback put every user's npm state in the chat hot
  ...
  of report (WinTone01/postillion#95): npm encodes fatal fs errors as `256 - errno`
...
- Replaced by `adapter_install`: pinned packages install ONCE into
  `~/.postillion/adapters/<pkg>/<version>` (`$POSTILLION_ADAPTERS_DIR` overrides) with
  a postillion-owned npm cache beside them, atomically (tmp dir + bin-entry
  verification + `.postillion-install-ok` marker + rename), then every launch is
