...
- EphemeralStore (loro::awareness::EphemeralStore): timestamp-LWW presence store, per-key timeout
  ...
  Rust and JS. (Postillion's %EPH presence maps to this.)
...
- Plan: build a small `postillion-mirror` crate: typed schema structs + incremental diff application over
...
- Sync caveat: peers can only sync if their version is after the shallow start — the session DO is
  the natural sequencer for compaction (postillion already does daily frontier checkpoints >= RETAIN_DAYS).
