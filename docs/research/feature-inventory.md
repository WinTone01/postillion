# Postillion — Complete Feature Inventory (parity checklist for the native rewrite)

Source: exploration of ~/github/postillion on 2026-07-19. Rewrite keeps ONLY the Loro path; token-usage
...
- NOTE: chats/sessions/devices/checkout-diffs entity sync is STILL Postgres-based in postillion — the rewrite
...
- External links open in OS browser; single-instance lock; dev vs packaged ports (26634/26654) and
  data dirs (.postillion-dev/.postillion). Fullscreen hides traffic lights -> cluster reflows.
...
- Boot Splash: postillion-wave loader, fades out (splash-out) once connected && authReady (15s cap),
...
- Pickers: HarnessModelPicker (harness rail + models, harness locked once chat exists),
  ...
  URL…, Create new repo…), BranchPicker (search + isolated-worktree toggle ~/.postillion/worktrees/…).
...
- postillion-pulse 2.4s infinite staggered cell opacity 0.08->1 scale 0.9->1
...
- 3.5 Repos/diffs: list/add/clone/create; branches; worktrees ~/.postillion/worktrees/<repo>/<name>;
