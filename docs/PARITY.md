...
| 1.6 Sidebar | done | Device switcher, new session, grouped-by-project or flat, status dots (staleness-checked), row context menu (rename/archive/delete), resort glide, "Sessions on this machine" palette (detected Claude Code transcripts → import with history + resume). |
...
| ListLocalChats / AdoptLocalChat | done | Postillion addition (v1 parity): scans this device's Claude Code transcripts and turns one into a chat with its history and `--resume` continuity. Relay-forwardable — the transcripts live on the device that ran them. |
...
| Wire types | done | `postillion-proto`: AgentEvent, ToolCall kinds, models/options, entities, AuthState. |

## §3 Backend engine

| Item | Status | Notes |
| --- | --- | --- |
| 3.1 Lifecycle | partial | Device registration, presence heartbeat (ephemeral, 15s), stale-session recovery, host-only doc executor with steer→new-turn fallback, single-instance data-dir lock. CLI auth decoupled from the daemon: `postillion login`/`logout`/`status` work on the persisted session and exit; headless TTY sign-in remains, and off-TTY (systemd/launchd) headless fails fast with "run `postillion login` first"; `postillion daemon install/start/stop/restart/status/uninstall` manages launchd / systemd `--user` units (install-time PATH captured into the unit for harness CLIs). Gaps: login-shell PATH capture for the headed app, crash shield, parent-PID watchdog. |
