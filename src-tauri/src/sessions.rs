        ...
        for project in fs::read_dir(&root)?.filter_map(|e| e.ok()) {
            ...
            for file in fs::read_dir(&project_path)?.filter_map(|e| e.ok()) {
                ...
                match self.load(&path) {
                    // Konuşma taşımayan transcript'ler listeye girmiyor:
                    // `claude -p "/usage"` gibi her çağrı bir tane bırakıyor
                    // ve kullanıcı için oturum sayılmıyorlar.
                    Ok(session) if session.has_conversation => out.push(session),
                    Ok(_) => continue,
        ...
        match v.get("type").and_then(|t| t.as_str()) {
            ...
            Some("user") => {
                    ...
                    .and_then(|c| c.as_str())
                {
                    ...
                    // `<local-command-stdout>` ve benzerleri. Kullanıcının
                    // yazdığı bir şey değiller.
                    let head = text.trim_start();
                    if head.starts_with("<command-") || head.starts_with("<local-command-") {
                        return;
                    }
                    ...
                    // Görüntü ya da araç sonucu taşıyan dizi biçimi.
                    s.has_conversation = true;
                }
            }
            Some("assistant") => {
...
"#,
    ...
    /// `claude -p "/usage"` çağrısının bıraktığı transcript'in birebir şekli
    /// (diskten alındı). Kullanıcının açtığı bir oturum değil.
    #[test]
    fn yalnizca_yerel_komut_iceren_transcript_konusma_sayilmaz() {
        let path = tmp_jsonl(
            ...
            concat!(
                ...
                "\n",
                r#"{"type":"user","isMeta":true,"message":{"role":"user","content":"<local-command-caveat>Caveat: The messages below were generated by the user while running local commands. DO NOT respond…</local-command-caveat>"}}"#,
                "\n",
                r#"{"type":"user","message":{"role":"user","content":"<command-name>/usage</command-name><command-message>usage</command-message>"}}"#,
                "\n",
        ...
        // Başlık meta kayıttan türetilmemeli.
        assert_eq!(s.title, None, "başlık: {:?}", s.title);
    }

    /// Gerçek bir mesajı olan oturum, içinde yerel komut kayıtları olsa da
    /// listede kalmalı.
    #[test]
    fn gercek_mesaj_varsa_konusma_sayilir() {
        let path = tmp_jsonl(
            "karisik.jsonl",
            concat!(
                ...
                "\n",
                r#"{"type":"user","message":{"role":"user","content":"merhaba"}}"#,
                "\n",
            ),
        );
        ...
        assert_eq!(s.title.as_deref(), Some("merhaba"));
    }

    /// Claude yerel ürettiği kayıtlara `<synthetic>` yazıyor; rozette model
    /// adı olarak görünüyordu.
    #[test]
    fn sentetik_model_rozete_gecmez() {
        let path = tmp_jsonl(
            "sentetik.jsonl",
            concat!(
                r#"{"type":"user","message":{"role":"user","content":"selam"}}"#,
                "\n",
