...
#
# Everything lives under /tmp/postillion-demo-*; re-runs reuse it. Ctrl-C cleans up.
set -euo pipefail
cd "$(dirname "$0")/.."

DAEMON_DIR=/tmp/postillion-demo-daemon
UI_DIR=/tmp/postillion-demo-ui
IPC=27921
DELAY=""
[[ "${1:-}" == "--slow" ]] && DELAY=350

echo "▸ building (first run takes a few minutes)…"
cargo build -p postillion -q

echo "▸ starting engine daemon on :$IPC"
env POSTILLION_DATA_DIR="$DAEMON_DIR" POSTILLION_IPC_PORT=$IPC POSTILLION_HARNESS=mock \
  ${DELAY:+POSTILLION_MOCK_DELAY_MS=$DELAY} RUST_LOG=warn \
  ./target/debug/postillion headless &
DAEMON_PID=$!
...
probe() { cargo run -q -p postillion-rpc --example rpc_probe -- "ws://127.0.0.1:$IPC" "$@"; }
...
if [[ ! -f "$DAEMON_DIR/.demo-seeded" ]]; then
  ...
  declare -A SPACES=()
  for project in postillion soccertcg postillion aether; do
  ...
  seed "Native Postillion Rust Rewrite"    postillion postillion/main                 0  run
  seed "Rebalance Player Stats Caps"  soccertcg    postillion/rebalance-player-stat-caps  2  run
  seed "Craft Premium TCG Experience" soccertcg    postillion/craft-premium-tcg-exp       26 skip
  seed "Initial Context Exploration"  postillion        postillion/initial-context-exploration 14 skip
  seed "Soccer TCG Repo Creation"     aether       aether/main                       48 skip
  touch "$DAEMON_DIR/.demo-seeded"
fi

echo "▸ opening postillion (composer is live — type into it; --slow shows streaming)"
POSTILLION_DATA_DIR="$UI_DIR" POSTILLION_IPC_PORT=$IPC RUST_LOG=warn ./target/debug/postillion
