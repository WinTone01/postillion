...
#   target/package/postillion-<version>-macos-<arch>.dmg          (user download)
#   target/package/postillion-<version>-macos-<arch>-app.tar.gz   (auto-updater)
# containing Postillion.app (unsigned unless CODESIGN_IDENTITY is set).
#
...
APP="$OUT_DIR/Postillion.app"
DMG="$OUT_DIR/postillion-$VERSION-macos-$ARCH.dmg"
APP_TARBALL="$OUT_DIR/postillion-$VERSION-macos-$ARCH-app.tar.gz"
...
install -m 755 "$ROOT/target/release/postillion" "$APP/Contents/MacOS/postillion"
...
# applied ahead of time; dist/postillion.png stays the full-bleed shared artwork).
ICONSET="$OUT_DIR/postillion.iconset"
...
iconutil -c icns "$ICONSET" -o "$APP/Contents/Resources/postillion.icns"
rm -rf "$ICONSET"
...
if $NOTARIZE; then
  ...
  # Gatekeeper offline.
  ZIP="$OUT_DIR/postillion-notarize.zip"
...
tar -czf "$APP_TARBALL" -C "$OUT_DIR" Postillion.app
...
dmgbuild.build_dmg(
    ...
    volume_name="Postillion",
    settings={
        "format": "UDZO",
        "files": [app],
        "symlinks": {"Applications": "/Applications"},
        "icon": os.path.join(app, "Contents/Resources/postillion.icns"),
        ...
        "icon_size": 104,
        "text_size": 12,
        "icon_locations": {"Postillion.app": (165, 195), "Applications": (495, 195)},
    },
)
PY
rm -f "$BG_TIFF"
