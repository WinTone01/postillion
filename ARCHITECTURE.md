# postillion — Architecture

A ground-up native rewrite of [postillion](../postillion) — a multi-device controller for coding agents
...
- Feature parity with postillion **except token-usage display** (poor fit for CRDTs; excluded).
...
- **Engine = backend** (was `@postillion/backend`): runs agents, owns auth, terminals, repos/worktrees,
...
### Headed / headless
Single binary `postillion`:
- `postillion` — headed. If a local engine daemon is already listening on the IPC port, connect to it;
...
- `postillion headless` — engine only. A clean installation immediately serves its local profile over localhost IPC; when a saved account selects the synced profile at startup and a bearer is available, it also hosts its DeviceRoom for remote control. A VPS can run this while a laptop's UI drives it.
...
`postillion login` and `postillion logout` operate on `session.json` while the engine is stopped. Login selects `Synced` for the next start; logout selects `Local` for the next start. The UI may update live authentication status, but the active `WorkspaceScope` still changes only after restart.
...
3. **Mirror layer** (`postillion-doc` crate) — Rust equivalent of loro-mirror: typed structs for the
...
postillion/
  ...
  crates/
    proto/        postillion-proto    # wire types: AgentEvent, ToolCall, RunRequest, Model,
    ...
    doc/          postillion-doc      # session-doc + workspace-registry schemas, mirror layer,
                                 # parts fold, continuations, command ledger, sidecars
    sync/         postillion-sync     # loro room client (join/VV backfill/fragments/backoff),
                                 # ephemeral presence, DocsStore (SQLite snapshots +
                                 # processed-command ledger)
    harness/      postillion-harness  # Harness trait + claude-code (stream-json subprocess),
                                 # codex (app-server JSON-RPC), mock; steering mailbox,
                                 # requestInput, models/reasoning/options catalogs
    engine/       postillion-engine   # sessions engine (pub/sub, run journal, recovery, stall
    ...
    rpc/          postillion-rpc      # UiRpc/ControlRpc: typed req/resp/stream over WS (tokio-
                                 # tungstenite) + in-memory transport; device-room virtual
                                 # sockets ({s,k,to,from} frames)
    ui/           postillion-ui       # gpui app: shell, sidebar, conversation, composer,
                                 # terminal view, diff pane, settings, animation kit
  apps/
    postillion/                       # the binary (headed default, `headless` subcommand)
...
- **Markdown** (`postillion-ui::markdown`): `pulldown-cmark` parsing on `background_spawn` with
...
- **Animation kit** (`postillion-ui::motion`): small helpers over gpui `Animation` reproducing the
  postillion catalog — `fade-in` (0.5s, cubic-bezier(0.16,1,0.3,1), translateY 4→0), `splash-out`,
  `postillion-pulse` staggered cell wave (boot splash + loaders), `gradient-spin-pulse` matrix
...
Direct ports of postillion behaviors (spec: feature-inventory §3):
...
- **Repos/diffs**: git2 or `git` subprocess (subprocess — matches postillion, avoids libgit2 edge
  cases); worktrees under `~/.postillion/worktrees`; fs watchers (`notify`) + 2min repair; diff
...
Port `postillion/apps/edge` nearly verbatim (it is already Loro-native and smoke-tested: session room
...
- **Changed**: Postgres entity sync/server → workspace registry + edge; Electron/React/mugen → gpui with
  ported techniques; Node harness SDKs → subprocess protocols; WebRTC → device-room relay (postillion
...
- ✅ **M1 Doc + sync core** — `postillion-doc` mirror over loro 1.13; room client syncs with the edge
...
- ✅ **M2 Engine core** — Claude harness end-to-end headless: `postillion headless` + dev auth runs a
...
3. Cursor harness (postillion has it; CLI surface for Rust TBD) — parity item, scheduled after Codex.
